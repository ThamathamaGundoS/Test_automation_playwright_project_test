...
import  bookingID from '../Utilities/Utilities.json' assert { type: 'json' };
...
test.describe('Restful Booker Booking CRUD operations', () => {
  ...
  test('Get Booking Details', async ({ request }) => {
    const book = bookingID.values()
    const response = await request.get(`${baseURL}${endpoints.booking}/${book}`, {
      headers: {
        Accept: 'application/json',
      },
    });
